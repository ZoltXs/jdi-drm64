...
// Matrices de dithering basadas en driver funcional
static const int ditherMatrix1[2][2] = {
	{ 0, 2 },
	{ 3, 1 }
};

static const int ditherMatrix2[2][2] = {
	{ 0, 8 },
	{ 12, 4 }
};

static const int ditherMatrix3[4][4] = {
	{ 0, 8, 2, 10 },
	{ 12, 4, 14, 6 },
	{ 3, 11, 1, 9 },
	{ 15, 7, 13, 5 }
...
static const int ditherMatrix4[4][4] = {
	{ 0, 32, 8, 40 },
	{ 48, 16, 56, 24 },
	{ 12, 44, 4, 36 },
	{ 60, 28, 52, 20 }
...
// Función para aplicar dithering
static inline int apply_dithering(int value, int x, int y, int dither_level) {
	if (dither_level == 0) return value;
	int threshold = 0;
	switch (dither_level) {
		case 1: threshold = ditherMatrix1[y % 2][x % 2] * 16; break;
		case 2: threshold = ditherMatrix2[y % 2][x % 2] * 16; break;
		case 3: threshold = ditherMatrix3[y % 4][x % 4] * 4; break;
		case 4: threshold = ditherMatrix4[y % 4][x % 4] * 4; break;
		default: return value;
	}
	return (value + threshold > 255) ? 255 : (value + threshold);
}
// ===== FIN IMPLEMENTACIÓN DITHERING =====
...
/* Global button state */
static struct work_struct g_button_work;
extern bool g_param_backlit;

static void button_work_handler(struct work_struct *work) {
	g_param_backlit = !g_param_backlit;
	printk(KERN_INFO "sharp_memory: Button pressed, backlight %s\n", g_param_backlit ? "ON" : "OFF");
}

static irqreturn_t button_irq_handler(int irq, void *dev_id) {
	static unsigned long last_press = 0;
	unsigned long now = jiffies;
	if (time_after(now, last_press + msecs_to_jiffies(200))) {
		schedule_work(&g_button_work);
		last_press = now;
	}
	return IRQ_HANDLED;
}
	...
	u8* buf, struct drm_framebuffer *fb, struct drm_rect const* clip)
{
	...
	if (g_param_color) {
		drm_fb_xrgb8888_to_rgb888(&dst, NULL, &vmap, fb, clip, &conv_state);
		...
		*result_len = sharp_memory_to_color_tagged(buf,
		...
		drm_fb_xrgb8888_to_gray8(&dst, NULL, &vmap, fb, clip, &conv_state);
...
int drm_probe(struct spi_device *spi)
{
	...
	/* Setup GPIO 17 button */
	/* Setup global button work */
	INIT_WORK(&g_button_work, button_work_handler);
	if (gpio_request(17, "sharp-button") == 0) {
		if (gpio_direction_input(17) == 0) {
			panel->button_irq = gpio_to_irq(17);
			if (panel->button_irq >= 0) {
				if (request_irq(panel->button_irq, button_irq_handler, IRQF_TRIGGER_FALLING, "sharp-button", panel) == 0) {
					dev_info(dev, "Button GPIO 17 initialized\n");
				} else {
					panel->button_irq = -1;
				}
			}
		...
		dev_warn(dev, "Failed to setup button GPIO 17\n");
	}
...
void drm_remove(struct spi_device *spi)
{
	...
	if (panel->button_irq >= 0) {
		free_irq(panel->button_irq, panel);
		cancel_work_sync(&g_button_work);
		gpio_free(17);
	}
